function LogisticFitCalculations
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%growth
%23 July 2017 Run
%Version 2.0 written 11:54 3 August 2017 . Updated to be faster and more user
%friendly
%Version 2.1 written 16:08 3 August 2017 Loads scaled files and calculates 
%number of cells on its own based on position
%Version 3.0 written 13:51 5 August 2017: implements exponential fit in
%same code, further commenting and bug fixes: Functional
...
%also apply exponential?
prompt = 'The code will implement a logistic growth fit. Would you also like to implement\n an exponential fit for controls? 1==yes 0 == no\n';
exp = input(prompt);

% if exponential analysis is to be completed
if exp ==1
   prompt = 'Would you like to save the exponential plot? (1=yes 0=no)\n';
   expimagesave = input(prompt);
   if expimagesave == 1 %determine file name and path and store them
       prompt = 'Enter name of file to be saved without extension and with single quotes\n';
       expimagename = input(prompt);
       expimagepath = uigetdir('Z:\ENG_BBCancer_Shared','Where would you like to store the image file?');
       expimagename = strcat(expimagepath,'\',expimagename);    
   end
end

%see if the user wants a spreadsheet of results
prompt = 'Would you like to write the data to an organized spreadsheet? (0=no 1=yes)\n';
excelwrite = input(prompt);
%if yes define the file name and paths
if excelwrite == 1
    prompt = 'What would you like to call the output file (omit extension and enclose in single quotes)\n';
    excelfilename = input(prompt);
    excelpath = uigetdir('Z:\ENG_BBCancer_Shared','Where would you like to store the excel file?');
end

%do a similar thing for logistic growth images
prompt = 'Would you like to save the logistic growth image? (1=yes 0=no)\n';
savestatus = input(prompt);
if savestatus == 1
   prompt = 'Enter name of file to be saved without extension and with single quotes\n';
   imagename = input(prompt);
   imagepath = uigetdir('Z:\ENG_BBCancer_Shared','Where would you like to store the logistic image file?');
   imagename = strcat(imagepath,'\',imagename);
end
disp('ALL PARAMETERS NOW ENTERED PLEASE WAIT FOR CODE COMPLETION')

tic
%create an empty matrix we will later fill with the final values
AggMat = zeros(numel(desiredwells),6);
storecounter = 1; %create a counter for the purposes of indexing

%define the color scheme
cmap = cbrewer('qual','Set1',numel(desiredwells));
colorcounter = 1; %create a color indexer

%implement dimensional checks, repairs, and error alerts
if size(desiredwells,1) == 1
else
    desiredwells = desiredwells';
    if size(desiredwells,1) == 1
    else
        error('PROGRAM CRASH: THE DESIRED WELLS MUST BE ENTERED AS A 1 by n MATRIX');
    end
end
...
for currentwell = desiredwells
    filename = strcat('Z:\ENG_BBCancer_Shared\group\0Zach\Cluster data\EGF (E6) Data',...
        '\egf(E6)newwell',num2str(currentwell),'.mat'); %set the new file name
    load(filename); %load that file
    
    %setup overall matrix from the position data
    overall = NaN(size(storeX,2),2); %#ok<NODEF>
    overall(:,2) = [1:size(storeX,2)]'; %#ok<NBRAK>
    for aa = 1:size(storeX,2)
        %figure out now many actual cells there are in a timepoint
        m = isnan(storeX(:,aa)) == 0;
        overall(aa,1) = sum(m);
    end
    ...
    %plot the data
    h = plot(cf,'r');
    %turn repetitive legend entries off
    set(get(get(h,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
    hold on %enable dual plotting 
    ...
    title('Logistic Growth Plot')
    
    %construct a variable custom array for the legend
    legendname = cell(1,numel(desiredwells));
    legendindex = 1; %create an index
    for aa = desiredwells %loop through the wells, storing each in the labeling cell
       legendname{1,legendindex} = strcat('Well',num2str(aa));
        legendindex = legendindex + 1;
    end
    
    legend(legendname,'location','nw')
    
    %write to an excel file
    if excelwrite == 1
        disp('PLOTTING NOW COMPLETE--WRITING EXCEL FILE');
    ...
    %Write to an Excel File
    ExcelFile = strcat(excelpath,'\',excelfilename,'.xlsx');
    writetable(Final,ExcelFile)
    end
    %...and an image
    if savestatus == 1
    disp('EXCEL FILE IS WRITTEN--SAVING HIGH QUALITY IMAGE FILES (THIS MAY TAKE SOME TIME)');
    print(gcf,imagename,'-depsc','-r2000')
    print(gcf,imagename,'-dpng','-r2000')
    end
    %alert the user of progresss
    disp('LOGISTIC GROWTH FIT IS NOW COMPLETED AND SAVED!');
    
    %IF the user wants to investigate standard exponential fits
    if exp == 1
        %open a new figure and reset the color counter
        figure; 
        colorcounter = 1;

        for currentwell = desiredwells %loop through the specified wells
            filename = strcat('Z:\ENG_BBCancer_Shared\group\0Zach\Cluster data\EGF (E6) Data',...
                '\egf(E6)newwell',num2str(currentwell),'.mat'); %set the new file name
            load(filename); %load that file

            %setup overall matrix from the position data
            overall = NaN(size(storeX,2),2);
            overall(:,2) = [1:size(storeX,2)]'; %#ok<NBRAK>
            for aa = 1:size(storeX,2)
                %figure out now many actual cells there are in a timepoint
                m = isnan(storeX(:,aa)) == 0;
            ...
            r = extractfield(r,'rsquare');
            disp(strcat('The Correlation Coefficient is',{' '},num2str(r),' For well ',num2str(currentwell)));
            vals = coeffvalues(z); %extract coefficient values
            disp(strcat('The Doubling time for well',{' '},num2str(currentwell),{' is '},num2str(log(2)/vals(2)),' hours'));
        ...
        if expimagesave == 1
        disp('SAVING HIGH QUALITY Exponential IMAGE FILES (THIS MAY TAKE SOME TIME)');
        print(gcf,expimagename,'-depsc','-r2000')
        print(gcf,expimagename,'-dpng','-r2000')
        end
        disp('The Exponential portion of the analysis is now complete')  
    end
    disp('CODE ANALYTICS NOW COMPLETE');
    toc
   load handel.mat
   sound(y);
